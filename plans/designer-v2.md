...
# 修订 A1（2026-07-17，依据真实装修项目「QR 案」全程 PJ 复盘）

**定位修正**：designer 管线不是获客装饰，是**前置风险歼灭器**。实证：9 起返工中 7 起、约 10 周延误 root cause 在设计阶段（riser 撞设计、TV 墙未敲检、窗帘轨道无位、as-built 尺寸真空、色彩原则被采购违反、渲染承诺不可建造、需求执行漂移）。**成功指标从渲染质量改为：返工率 + 决策稳定性**。同时确立 PJ 台账为主产品、designer 为前置模块的总排序；WP3-5 保留但降级，以下四条增补插队。

## A1.1 前置勘验门（L1 扩展到 services 层）

设计解锁的前置条件：**现场勘验 checklist 完成并入库**，缺项设计不开工。
必查项：gas riser / 立管位置与可动性（向 SP/City Energy 书面确认，不凭经验赌）、梁柱实位（对照 L2 推断逐一确认）、每面涉及挂装/开洞的墙体敲检（实心/空心/老化度）、窗帘/百叶轨道空间预留、地漏与排水管走向、既有设备状况（空调管线、门框）。
每项 = 一条 L1 fact（带照片证据），review_required 不清零 → 设计层锁死。
（对照案例：riser 1 月才发现 = 损失 4 周；TV 墙 7 月还没解决。）

## A1.2 L3 设计原则结构化（原则即数据）

用户审美原则从聊天流升格为 versioned 数据：
```json
{"id":"principle.chroma-family","claim":"全屋同色度家族，禁止冷暖对撞","status":"user_confirmed","examples":["黑色 bifold 违例 → 4-07 返工案"]}
```
**审计范围覆盖全部采购决策，不只渲染**——vendor 选品（门、五金、laminate、灯具）逐项过原则审计，违例在下单前拦截，不是安装后由用户肉眼发现。原则违例 = B2 审计 fatal 级。

## A1.3 预算 = 决策级 hook

每个设计/变更决策入台账时**必须带 price 字段**，无价格的决策标记 pending-quote 并自动催办。运行时效果：
- 实时 running total 对合同额百分比可见（本案 V/O 达 48% 才被看见）
- V/O = 台账自动 diff，lump sum 结构性消失
- 正反面实证：窗户 +$550 决策当下报价 → 零纠纷；sliding doors 2-04 问价、4-02 才随 lump sum 出现 $3,100 → 对账冲突

## A1.4 渲染可建造性审计（B1 扩展）

- 渲染禁止展示 vendor 体系交付不了的元素：无框淋浴（10mm 玻璃需支撑杆/稳定臂）、无缝色匹配（粉末喷涂≠油漆）、忽略结构件的净空
- 引入 **vendor 约束库**（玻璃/门/窗五金体系的可交付形态），渲染出图前过一遍
- **渲染变更 = 决策变更**：任何已批准渲染的修改（如淋浴门 swing 方向）触发用户显式确认，未确认不施工
（案例：3D 无框淋浴 vs 实物齐头顶杆、swing in/out 罗生门、瓷砖两次 "render bugged"。）

## A1.5 通话黑洞四层方案（PJ 模块，按 ROI 排序）

诊断：承包商在高 stakes 时刻（危机/坏消息/大额决策）系统性转入电话，台账在最关键处失明；且一半以上"紧急"来电用事实层+原则层即可代答。

| 层 | 方案 | 依赖 | 状态 |
|---|---|---|---|
| 1 | **Post-call recap**：通话后用户 30 秒语音复述 → 转写结构化 → 起草追认消息发群，对方确认后口头决策回流台账 | 无 | 本周可加入 PJ 流程 |
| 2 | **Voice-note-first**：引导承包商"别 call 发语音"（对其零成本+异步+天然留痕），语音自动转写入台账 | ASR 管线（已验证） | 本周 |
| 3 | **决策预授权矩阵**：≤$X 且可逆且不违反 L3 原则 → agent 直接代答；graduated autonomy 用于需求侧 | A1.2 原则层 | 排队 |
| 4 | **Decision packet**：真升级先自动收齐现场照片/选项/价格/deadline，用户收到 30 秒可拍板的决策包而非"快回电" | 1-3 | 成熟形态 |

v1 不做通话录音/AI 入会（同意摩擦大、关系型承包商观感差），recap 回路拿 80% 价值。

## A1.6 商业化排序备忘（本次复盘结论）

楔子 = V/O 审计单品（上传报价+V/O → 抓重复计费，ROI 当场可量化）→ 主产品 = per-project async PJ（导出/转发式摄入，不做实时桥）→ 长线 = 供给侧 OS + vendor 已验证履约数据资产。C 端低频（5-10 年一次）决定其永远是获客层与数据层；"放手"约束下优先 C 端自助漏斗。
